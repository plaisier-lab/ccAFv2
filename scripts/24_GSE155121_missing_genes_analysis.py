...
for cutoff1 in cutoffs:
    ...
    # Save out data
    DF.to_csv(savedir+'/'+tag1+'_'+ws1+'_ccAFv2_CV_missing_genes_analysis_'+str(cutoff1)+'.csv')


# Read in data for plotting
ccSeurat_calls = ccAF1_scanpy.obs['Phase']
extra_col = list(ccSeurat_calls)*10
cutoffs = [0.5]
#cutoffs = [0.5, 0.7, 0.9]
percs = [0.99, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1]
input = {}
...
cells_predicted = {}
for cutoff1 in cutoffs:
    file1 = pd.read_csv(savedir+'/'+tag1+'_'+ws1+'_ccAFv2_CV_missing_genes_analysis_'+str(cutoff1)+'.csv')
...
df2.to_csv(savedir+'/'+tag1+'_'+ws1+'_percentage_genes_remove_unknowns_'+str(cutoff1)+'.csv')
# Together plots
...
ax1.set_ylim(0,1)
plt.savefig(savedir+'/'+tag1+'_'+ws1+'_percentage_genes_boxplot_together_remove_unknowns_'+str(cutoff1)+'.pdf')
plt.clf()
...
df3.index = percs
df3.to_csv(savedir+'/'+tag1+'_'+ws1+'_all_metrics_percentage_genes_sensitivity_medians_'+str(cutoff1)+'.csv')
