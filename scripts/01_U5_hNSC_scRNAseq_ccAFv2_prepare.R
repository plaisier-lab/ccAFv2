...
#docker run -it -v '/home/soconnor/old_home/ccAF/:/files' saoconn1/scrnaseq_latest
...
dir.create(file.path(tag, save_dir), showWarnings = FALSE)
resdir2 = file.path(tag, save_dir)

# Create folder to save out seurat objects (unfiltered, filtered, normalized, etc.)
obj1 = 'seurat_objects'
dir.create(file.path(resdir2, obj1), showWarnings = FALSE)
resdir3 = file.path(resdir2, obj1)
...
#data <- Read10X(file.path(tag, data_dir), gene.column=1) # column 1 is ensembl IDs (in 10X mtx file)
#dim(data)
#------------------------------------------------------------------------------#
...
data <- Read10X(file.path(tag, data_dir), gene.column=2) # column 2 is gene symbols (in 10X mtx file)
dim(data)
# U5: 32738  3049
...
ccAF <- read_csv(file.path(tag, paste0(tag, "_ccAF_cells.csv")))
...
#---------------------------------------------------
#------ USE ENSEMBL IDs IF WANT TO SAVE OUT AS LOOM FILE TO APPLY CCAF --------#
#mitogenes <- read_csv("U5/mito_genes.csv")
...
save_dir1 = "final"
dir.create(file.path(resdir3, save_dir1), showWarnings = FALSE)
resdir4 = file.path(resdir3, save_dir1)

# Gene symbols
saveRDS(seurat1, file.path(resdir4, paste0(tag, "_filtered_gene_symbols.rds")))
SaveH5Seurat(seurat1, file.path(resdir4, paste0(tag, "_filtered_gene_symbols.h5Seurat")), overwrite = TRUE)
Convert(file.path(resdir4, paste0(tag, "_filtered_gene_symbols.h5Seurat")), dest = "h5ad", overwrite = TRUE)
data_loom <- as.loom(seurat1, file.path(resdir4, paste0(tag, "_filtered_gene_symbols.loom")), verbose = FALSE, overwrite = TRUE)
data_loom$close_all()

# Ensembl IDs
#saveRDS(seurat1, file.path(resdir4, paste0(tag, "_filtered_ensembl.rds")))
#SaveH5Seurat(seurat1, file.path(resdir4, paste0(tag, "_filtered_ensembl.h5Seurat")), overwrite = TRUE)
#Convert(file.path(resdir4, paste0(tag, "_filtered_ensembl.h5Seurat")), dest = "h5ad", overwrite = TRUE)
#data_loom <- as.loom(seurat1, file.path(resdir4, paste0(tag, "_filtered_ensembl.loom")), verbose = FALSE, overwrite = TRUE)
#data_loom$close_all()
...
seurat2 <- RunUMAP(seurat2, dims=1:5, verbose = FALSE)

# Save out seurat object
# gene symbols
saveRDS(seurat2, file.path(resdir4, paste0(tag, "_normalized_gene_symbols.rds")))
SaveH5Seurat(seurat2, overwrite = TRUE, file.path(resdir4, paste0(tag, "_normalized_gene_symbols.h5Seurat")))
Convert(file.path(resdir4, paste0(tag, "_normalized_gene_symbols.h5Seurat")), overwrite = TRUE, dest = "h5ad")
data_loom <- as.loom(seurat2, file.path(resdir4, paste0(tag, "_normalized_gene_symbols.loom")), verbose = FALSE, overwrite = TRUE)
data_loom$close_all()

# ensembl
#saveRDS(seurat2, file.path(resdir4, paste0(tag, "_normalized_ensembl.rds")))
#SaveH5Seurat(seurat2, overwrite = TRUE, file.path(resdir4, paste0(tag, "_normalized_ensembl.h5Seurat")))
#Convert(file.path(resdir4, paste0(tag, "_normalized_ensembl.h5Seurat")), overwrite = TRUE, dest = "h5ad")
#data_loom <- as.loom(seurat2, file.path(resdir4, paste0(tag, "_normalized_ensembl.loom")), verbose = FALSE, overwrite = TRUE)
#data_loom$close_all()


#------------------------------------------------------
# Read in processed data all together
#---------------------------------------------------
...
for(tag in tags){
  ...
  resdir2 = file.path(tag, save_dir)
  save_dir1 = 'final'
  dir.create(file.path(resdir2, save_dir1), showWarnings = FALSE)
  resdir3 = file.path(resdir2, save_dir1)
  #write.csv(Embeddings(object = seurat2[["umap"]]), file.path(resdir3, paste0(tag, "_UMAP_Coordinates_for_scvelo.csv")))
  ...
  write.csv(cluster_markers, file.path(resdir3, paste0(tag,"_scTransform_Markers_together.csv")))
  write_xlsx(sp, path = file.path(resdir3, paste0(tag,"_scTransform_Markers_by_cluster.xlsx")))
  cluster_markers %>%
      group_by(cluster) %>%
  ...
  saveRDS(seurat2 , file.path(resdir3, paste0(tag, "_seurat2.rds")))
  # Dump out pdf of plots
  pdf(file.path(resdir3, paste0(tag, "_v2.pdf")), width =8, height = 8)
  # umaps
  lst = list(c1)
  grid.arrange(grobs = lst, layout_matrix = rbind(c(1,NA), c(NA,NA)), top = "")
  lst2 = list( o1, o2, o4)
  grid.arrange(grobs = lst2, layout_matrix = rbind(c(1,2), c(3)), top = "")
  # Heatmap
  print(DoHeatmap(object = seurat2, features = top10$gene))
  # ccAF Heatmap
  print(DoHeatmap(object = seurat2, features=unlist(goi_lst), group.colors = list(data.frame(ccAFv2_colors[sub1])$ccAFv2_colors.sub1.)[[1]]) + ggtitle("ccAF genes"))
  # Cylin ridgeplots
