...
#---------------------------------------------
# Load data and subset genes to marker genes
#--------------------------------------------
datasets = {}
for set1 in sets:
...
calls = pd.read_csv('U5_ccAFv2_calls_020224.csv', index_col = 0)
# Set column name
ccAF1_scanpy.obs['ccAFv2'] = calls['x']
tmp = ccAF1_scanpy[ccAF1_scanpy.obs['ccAFv2'] != 'Unknown']
...
for cutoff1 in cutoffs:
    ...
    # Save out data
    DF.to_csv('results/SensitivityAnalysis/final_cutoff/ccAFv2_CV_sensitivity_analysis_'+str(cutoff1)+'_031224.csv')


# Read in data for plotting
resdir9 = 'results/SensitivityAnalysis/final_cutoff'
nfolds = 10
labs = pd.read_csv('U5_ccseurat_calls_101023.csv')
labs['x'].replace({'G2M': 'G2/M'}, inplace=True)
extra_col = list(labs['x'])*10
#cutoffs = [0.5, 0.7, 0.9]
cutoffs = [0.5]
percs = [0.99, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1]
...
error_rate = {}
for cutoff1 in cutoffs:
    file1 = pd.read_csv(resdir9+'/ccAFv2_CV_sensitivity_analysis_'+str(cutoff1)+'_031224.csv')
...
df2.to_csv(resdir9+'U5_percentage_genes_remove_unknowns_'+str(cutoff1)+'_031224.csv')
# Together plots
...
plt.savefig(resdir9+'/U5_percentage_genes_boxplot_together_remove_unknowns_'+str(cutoff1)+'_031224.pdf')
plt.clf()
...
df3.index = percs
df3.to_csv(resdir9+'/U5_all_metrics_percentage_genes_sensitivity_medians_'+str(cutoff1)+'_031224.csv')
